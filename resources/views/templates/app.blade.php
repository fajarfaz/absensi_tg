...
<body>
            ...
            <div class="header-sidebar bg_white_color is-fixed">
                <div class="tf-container">
                    <div class="d-flex justify-content-between align-items-center">
                        <a href="{{ url('/') }}" class="sidebar-logo d-flex align-items-center">

                                <img src="{{ asset('/storage/'.$settings->logo) }}" alt="logo">
                                <h5 class="ms-3">{{$settings->name}}</h5>
                ...
                <div class="tf-container">
                    <div class="box-content">
                        ...
                        <ul class="box-nav">
                            ...
                            <li>
                                <a href="https://wa.me/{{$settings->phone}}" class="nav-link">
                                    <i class="fas fa-question-circle" style="{{ Request::is('bantuan*') ? 'color: blue' : 'color: black' }}"></i>
                                    <span style="{{ Request::is('bantuan*') ? 'color: blue' : '' }}">Butuh Bantuan?</span>
                                </a>
                            </li>
                            <li>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            // Inisialisasi Swiper untuk setiap tab dengan autoplay nonaktif saat pertama kali dimuat
            const informasiSwiper = new Swiper('#informasi-swiper', {
                loop: true,
                autoplay: false,  // Nonaktifkan autoplay saat memuat
            });
            ...
            document.querySelectorAll('[data-bs-toggle="tab"]').forEach(tab => {
                tab.addEventListener('shown.bs.tab', function (event) {
                    const activeTab = event.target.getAttribute('href'); // Mendapatkan ID tab yang aktif

                    // Matikan autoplay di semua swiper
                    informasiSwiper.autoplay.stop();
                    newsSwiper.autoplay.stop();
                    eventsSwiper.autoplay.stop();

                    // Aktifkan autoplay hanya di swiper sesuai tab aktif
            ...
            // Jalankan autoplay pertama kali saat halaman dimuat (untuk tab Informasi)
            informasiSwiper.autoplay.start();
        });
    ...
    <script>
            ...
            window.Echo.channel("messages").listen("AnnouncementPublished", (event) => {
