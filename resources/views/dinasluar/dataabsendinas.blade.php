                    ...
                    <div class="table-responsive">
                        <table id="mytable" class="table table-striped">
                            ...
                            <tr>
                                ...
                                @if(isset($da->User->name))
                                <td>{{ $da->User->name }}</td>
                                @elseif(empty($da->User->name))
                                <td>{{ "User dengan ID " . $da->user_id . " telah di hapus." }} <strong>Apakah anda ingin menghapus data ini?</strong>
                                    <ul class="action">

                                        <li class="delete">
                                            <form action="{{ url('/data-dinas-luar/delete/'.$da->id) }}" method="post">
                                                @method('delete')
                                                @csrf
                                                <button title="Delete Pegawai" class="border-0" style="background-color: transparent;" onClick="return confirm('Jika Anda menghapus data ini, data rekap akan hilang! Apakah anda ingin melanjutkan?')"><i class="icon-trash"></i></button>
                                            </form>
                                        </li>
                                    </ul>
                                </td>
