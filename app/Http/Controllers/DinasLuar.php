...
class DinasLuar extends Controller
{
    ...
    public function deleteDinasLuar(Request $request, $id) {
        $dinasLuar = ModelsDinasLuar::find($id);

        if ($dinasLuar) {
            $dinasLuar->delete();

            return redirect('data-dinas-luar')->with('success', 'Data berhasil dihapus');
        }

        return redirect('data-dinas-luar')->with('error', 'Data tidak ditemukan');
